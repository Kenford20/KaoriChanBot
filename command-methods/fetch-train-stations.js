...
module.exports = async function fetchTrainStations(colorCode) {
  ...
  try { 
    const trainData = await fetch(CTA_getTrainStations_API, {
      method: "GET",
      header: {"Content-Type": "application/json"}
    }).then(response => response.json());
    
    if(trainData.length !== 0 && !trainData.error) {
      ...
      const stationNames = trainData.map(station => station.stop_name.replace(/ \(.+\)/, ''));
      const uniqueStops = stationNames.filter((stop, i) => stationNames.indexOf(stop) === i);
      const stationIDs = trainData.map(station => station.map_id);
